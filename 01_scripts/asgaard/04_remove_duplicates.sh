...
ls -1 "$ALIGNEDFOLDER"/*.bam |
    parallel -j 5 echo "Deduplicating sample {}" \; java -jar "$MARKDUPS" INPUT={} OUTPUT="$DEDUPFOLDER"/{/}.dedup.bam METRICS_FILE="$METRICSFOLDER"/metrics.txt VALIDATION_STRINGENCY=SILENT REMOVE_DUPLICATES=true MAX_FILE_HANDLES=100 TMP_DIR="./tmp"
#done
