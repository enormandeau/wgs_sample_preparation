...
## With GNU Parallel
# ls -1 samples_split/* | parallel -k -j 10 ./01_scripts/02_bwa-mem2_align_reads_PE_with_sample_file.sh 4 {}

## With GNU Parallel on SLURM
# ls -1 samples_split/* | parallel -k -j 10 srun -c 4 --mem 20G -p large --time 21-00:00 -J bwaMem -o 99_log_files/bwaMEMsplit_%j.log ./01_scripts/02_bwa-mem2_align_reads_PE_with_sample_file.sh 4 {} \; sleep 0.1 &
...
# Iterate over sequence file pairs and map with bwa-mem2
cat "$SAMPLE_FILE" |
while read file
do
    ...
    # Align reads
    bwa-mem2.avx mem -t "$NCPU" -R "$ID" "$GENOMEFOLDER"/"$GENOME" "$RAWDATAFOLDER"/"$name" "$RAWDATAFOLDER"/"$name2" |
